...
[Link](../presentations/tutorial02/index.html) to slides.
...
<p style="text-align: center">
    <img src="../fig/cmp3-diagram.png" style="width: 100%" alt="CMP3 diagram">
</p>
...
For enhanced accessibility, CMP3 comes with a Graphical User Interface aka `cmpbidsappmanager`, designed to facilitate the configuration of all pipeline stages, the configuration of the BIDS App run, the generation of the command, and its execution, and the inspection of the different stage outputs with appropriate viewers.
...
*   As illustrated by the previous Figure, CMP3 provides the three following pipelines:

    *   **Anatomical pipeline** computes the different cortical parcellations from anatomical MRI data.

    *   **Diffusion pipeline** computes the different structural brain connectivity matrices from diffusion MRI data.

    *   **fMRI pipeline** computes the different brain functional connectivity matrices based on the Pearson's correlation between time-series of the cortical parcels.
...
In this part of this tutorial, you will have exercises to learn how to configure the different pipelines and run CMP3 to produce the 5-scale Lausanne2018 brain parcellations and the structural brain connectivity matrices that will be used in the next tutorials.

CMP3 is installed in the virtual machine provided for the summer school along with miniconda3 and the following viewers:
...
**Note:** You will need to provide your own license at first use of `trackvis`. You can get one on the official [Download page](http://www.trackvis.org/download/). You will also need to provide your own Freesurfer license to use both CMP3 and `freeview`. Please check the [FreeSurfer Download and Registration page](https://surfer.nmr.mgh.harvard.edu/registration.html) to obtain one.
